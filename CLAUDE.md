...
- **Keep comments concise, precise, and non-obvious** — one or two lines, explaining *why*, never restating what the code does. Don't comment code whose behavior is already clear from the name and structure (`// increment counter` above `count++` is noise, delete it). If a comment needs a paragraph, fix the name or the code instead. No banner headings, no commented-out code, no change logs (that belongs in the commit message). A genuinely non-obvious constraint — a spec quirk, a browser bug, a deliberate tradeoff — can run longer; that's the exception, not the default.
...
- **Cleanup**: daily cron (3 AM UTC, configured in `wrangler.jsonc`) deletes annotations 90 days after their last access, plus any past an explicit `expires_at` or `owner_expires_at`.
- **The landing page's live demo**: the product screenshot on `/` is a poster over an iframe of the real viewer on a fixed public room (`DEMO_ROOM` in `packages/types`), so visitors draw on live Wikipedia with the actual toolbar and see each other. `GET /s/:id` mints that room if it is missing; an hourly cron (`0 * * * *`) wipes it through the Durable Object's `POST /reset` (never straight in D1 — a warm room would flush the old ops back). The room is embedded with `?still=1`, which stops the page inside from scrolling so a wheel over the window chains to the landing page (one scroller per screen). The landing's own overlays drop to `navigate` when the pointer crosses into the window, its drawn cursor yields there, and its fixed toolbar fades while the window holds half the screen (`demoInView`), so only one toolbar and one cursor are ever on screen.
- **D1 migrations, and the two names for one database**: `migrations/` replaced a hand-maintained `schema.sql` whose `ALTER TABLE`s were applied to production by hand. Prod never held wrangler's `d1_migrations` bookkeeping table, so `scripts/bootstrap-migrations.mjs` (`bun run migrations:bootstrap -- --remote`) reconstructed it from the live schema — that ran on 2026-09-08 and recorded 0001–0006, so a plain `migrations apply --remote` is the normal path now. The script stays because it is idempotent and answers "what does prod actually have" in one read. The naming trap is live, though: `d1 execute` and `d1 export` need Cloudflare's registered name `annotateweb`, while every `d1 migrations` subcommand resolves `database_name` from `wrangler.jsonc` and so wants `marklayer`.
